"""Pin `blemees.__version__` to `pyproject.toml`'s declared version.
...
def test_runtime_version_matches_pyproject():
    ...
    with pyproject.open("rb") as f:
        cfg = tomllib.load(f)
    assert blemees.__version__ == cfg["project"]["version"], (
        f"blemees.__version__={blemees.__version__!r} but pyproject declares "
...
def test_runtime_version_is_not_unknown_sentinel():
    ...
    assert blemees.__version__ != "0.0.0+unknown", (
