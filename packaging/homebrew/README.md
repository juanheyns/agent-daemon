...
```bash
gh repo create blemees/homebrew-tap --public --clone
cd homebrew-blemees
mkdir -p Formula
cp ../blemees-daemon/packaging/homebrew/blemees.rb Formula/
...
2. Update the `url` line to the new tag:
   `https://github.com/blemees/blemees-daemon/archive/refs/tags/vX.Y.Z.tar.gz`
3. Recompute the sha256:

   ```bash
   curl -sL https://github.com/blemees/blemees-daemon/archive/refs/tags/vX.Y.Z.tar.gz | shasum -a 256
   ```
